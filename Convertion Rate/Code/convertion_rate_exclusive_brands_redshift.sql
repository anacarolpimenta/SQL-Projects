...
, base as (
	select
		...
		, case 
			when bfoeb.first_order_eb is not null -- if not null means that the first exclusive brand order exists, so we count 
			then 1 
			else 0 
...
from base 
where datediff(month, order_date, current_date) <= 25 -- we want to query only the las 25 months of data to minimize the volume and make the dataset and the report more performatic.
group by 1,2,3,4
